...
class StudentReg():
	...
	def openImage(self):
		try:
			...
			original_stdn_pic = Image.open(self.img_file)
			resized_stdn_pic = original_stdn_pic.resize((80, 80),Image.ANTIALIAS)
			self.stdn_img = ImageTk.PhotoImage(resized_stdn_pic,master=self.window)
	...
	def openSign(self):
		try:
			...
			self.stdn_sign = ImageTk.PhotoImage(resized_stdn_sign,master=self.window)
	...
	def addData(self):
		...
		self.state_var.set(""),self.dist_var.set(""),self.pin_var.set(""),self.address.delete("1.0",END),self.lblsimg.config(image=""),self.lblsign.config(image="")
